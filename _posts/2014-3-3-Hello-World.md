...
To see this, let $$k$$ be the combined number of copies of Escape Plan and Prepared in the player's deck.  We suppose first that $$t(i) = 1$$.  Since Silent's deck begins with 12 basic cards and Ascender's Bane, and by floor 6, only 2 card removals are possible, the player's deck has at least $$11+k$$ cards.  Furthermore, noting the relics available, at most $$7+k$$ cards are drawn on turn $$1$$.  So $$t(j) > 1.5$$ as desired.  

Otherwise, $$t(i) \geq 2$$ and we consider the shuffle at time $$t(i)$$.  First, note that the at any given time during combat, the combined number of cards in the player's hand, discard pile, and draw pile is at least $$10 + k$$.  Indeed, the Silent starter deck has 13 cards including Ascender's Bane, only Ascender's Bane can be exhausted, and at most 2 starter cards can be removed prior to combat.  Since $$t(i) \geq 2$$, turn $$\lfloor t\rfloor$$ begins with the player drawing 5 cards, and no cards can be played to increase the number of cards in-hand, the player's hand consists of at most $$5$$ cards just before the shuffle at time $$t(i)$$.  If $$t(i)$$ is an integer, i.e., $$t(i)$$ corresponds to the shuffle at the beginning of turn $$t(i)$$, then turn $$t(i)$$ begins with at least $$5+k$$ cards in it.  On turns $$t(i)$$ and $$t(i)+1$$, at most $$k$$ draw cards (Escape Plan and Prepared) can be played, and it follows that no shuffle occurs at time $$t(i)+1/2, t(i)+1$$, or $$t(i)+3/2$$.  
