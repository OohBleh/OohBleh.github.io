...
If $$s$$ is an integer, i.e., $$s$$ corresponds to the shuffle at the beginning of turn $$s = \lfloor s\rfloor$$, then turn $$s$$ begins with at least $$5+k$$ cards in the draw pile.  On turns $$s$$ and $$s+1$$, at most $$k$$ draw cards (Escape Plan and Prepared) can be played, and it follows that no shuffle occurs at time $$s+1/2, s+1$$, or $$s+3/2$$.  So $$t-s\geq 2$$ as desired.  

Otherwise, $$r := s-1/2$ = \lfloor s\rfloor$$ is an integer and $$s$$ corresponds to a shuffle in the middle of turn $$r$$.  Since the player has at most $$5$$ cards in-hand just before the shuffle at time $$s$$, and at least one of these cards is a draw card, it follows that turn $$r$$ ends with one draw card and at most $$5$$ other cards in the discard pile.  This implies that the draw pile consists of at least $$10-5 = 5$$ non-draw cards at the end of turn $$r$$.  It follows that no other shuffle occurs at times $$s$$, $$s+1/2$$, or $$s+1$$.  So $$t-s\geq 2$$ as desired.  
